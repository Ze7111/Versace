...
```json
{
    ...
    "tasks": [
        {
            "label": "Run Versace File",
            "type": "shell",
            "command": "python versace.py ${relativeFile}",
        },
        {
            "label": "Decompile Versace File",
            "type": "shell",
            "command": "python versace.py ${relativeFile} True",
        }
    ]
}
```
...
- Now open the command palette again and type `Run Task` and select `Run Versace File` or `Decompile Versace File` depending on what you want to do.
...
## Code that MUST be in every Versace file
...
### Importing
- You can import python modules using the `include` keyword. (in the future you will be able to import other versace files)
...
### Any python syntax
- You can use any python syntax in the versace file, and it will work as expected.
- As long as you replace all the `:` with `{` and the indented part with `}`.
- Here is an example of a python function in versace:
```python
def say_hi() {
    print("Hello")
}
